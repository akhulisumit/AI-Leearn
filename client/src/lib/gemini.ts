...
export interface StudyBreakResponse {
  success: boolean;
  message: string;
  recommendation?: BreakRecommendation;
}

export async function getStudyBreakRecommendation(
  sessionTime: number, // in seconds
  topic: string,
  lastBreakType?: string
): Promise<StudyBreakResponse> {
  try {
    const response = await apiRequest('POST', '/api/study-break', {
      sessionTime,
      topic,
      lastBreakType
    });
    ...
    console.error("Error getting study break recommendation:", error);
    return {
      success: false,
      message: "Failed to get break recommendation. Please try again later."
    };
  }
}
